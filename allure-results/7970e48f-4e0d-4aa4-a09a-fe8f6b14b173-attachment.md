...
```
TypeError: productText?.trim is not a function
```
...
```ts
  ...
  16 |             const productText =product.locator('b').innerText()   
  17 | 
> 18 |             if (productText?.trim().includes(productName)) {
     |                              ^ TypeError: productText?.trim is not a function
  19 |                 await product.locator('button.btn.w-10.rounded').click()
  20 |                 break
  21 |             }
  22 |         }
  ...
  28 |         await this.myCart.waitFor({state: 'visible'})
  29 |     }
  30 | }
```